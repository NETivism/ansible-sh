...
function check_status() {
  ...
  case "$STATUS_CODE" in
    ...
    22)
      ...
      RESULT=$?
      if [ $RESULT -eq 0 ]; then
        jq -c '.status=2' $JSON_FILE > /tmp/$DOMAIN && mv /tmp/$DOMAIN $JSON_FILE && chown www-data $JSON_FILE
    ...
    33)
      ...
      RESULT=$?
      if [ $RESULT -eq 0 ]; then
        jq -c '.status=3' $JSON_FILE > /tmp/$DOMAIN && mv /tmp/$DOMAIN $JSON_FILE && chown www-data $JSON_FILE
